...
pub(crate) fn exchange_keys(stream: &mut TcpStream, role: &Role) -> (SessionKey, SessionKey) {
    ...
    stream.read_exact(&mut buf).expect("reading public_key failed");
    ...
    let my_session_keys = match role {
        Role::Server =>
            Session::new_server_with_defaults(&my_keypair, &partner_public_key).expect("compute session failed"),
        Role::Client =>
            Session::new_client_with_defaults(&my_keypair, &partner_public_key).expect("compute session failed"),
    };
