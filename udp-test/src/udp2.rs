...
const MSG_RESEND_DELAY: Duration = Duration::from_secs(1);
...
impl WaitingConnection {
    ...
    pub fn connect(self, peer: Ipv6Addr, port: u16) -> Result<ActiveConnection, ()> {
        ...
        return Ok(ActiveConnection::new(self.udp_socket).unwrap());
    }
...
impl ActiveConnection {
    fn new(udp_socket: UdpSocket) -> Result<ActiveConnection, ()> {
        ...
        if receiver_socket.set_read_timeout(None).is_err() {
            return Err(());
        }
        ...
        return Ok(ActiveConnection {udp_socket, send_counter: 0, acknowledgement_receiver,message_receiver, receiver_handle});
    }
...
mod tests {
    ...
    fn prepare_local() -> (ActiveConnection, ActiveConnection) {
        ...
        let mut c1 = w1.connect(ipv6, p2).unwrap();
        let mut c2 = w2.connect(ipv6, p1).unwrap();
