...
pub fn create_data_vec(path:&str, order:&Order, chunk_num:usize) -> Result<Vec<u8>, RError>{
    ...
    let mut header = create_header(file_size, max_chunk_count, &order.file_hash_type, &Some(CHUNK_HASH_TYPE));
    let split_vec = split_file_single(&mut buf_reader, chunk_num, file_size as usize, order.chunk_size, &order.file_hash, max_chunk_count, &USER_HASH, &mut header, &Some(CHUNK_HASH_TYPE))?;
