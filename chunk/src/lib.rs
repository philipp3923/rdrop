...
use offer::offer::{read_offer_vec, Offer};
use order::order::{create_order_from_offer, Order};

use crate::file::file::r_w_data_vec;
use crate::general::general::{validate_file, BUFFER_SIZE, CHUNK_HASH_TYPE, CHUNK_SIZE};
use crate::hash::hash::Hash;
use crate::order::order::{create_order_from_logfile, read_order};

pub fn read_offer_byte_vec(byte_vec: &Vec<u8>) -> Result<Offer, RError> {
    return read_offer_vec(byte_vec);
}

pub fn create_order_from_offer_byte_vec(
    file_hash_alg: &Hash,
    output_dir: &str,
    offer: &Offer,
