use std::{io::{ErrorKind, Error, Write}, fs::{create_dir_all, File}, path::PathBuf};
...
impl Order{
    pub fn new(chunk_size:&str, file_hash_type:&str, file_hash:&str, file_name:&str, start_num:&str, end_num:&str) -> Result<Self, Error>{
        ...
        let file_hash_type = match file_hash_type{
            ...
            "SHA512" => Hash::SHA512,
            _ => {
                return Err(Error::new(ErrorKind::InvalidInput, "Hash-Alforithm not implemented."));
            }
        };
...
pub fn read_order(byte_vec:&mut Vec<u8>) -> Result<Order, RError>{
    ...
    let regex = Regex::new(ORDER_REGEX).map_err(|err| RError::new(RErrorKind::RegexError, &err.to_string()))?;
    ...
    if let Some(captures) = regex.captures(&order) {
        ...
        let order = Order::new(chunk_size, file_hash_type, file_hash, file_name, start_number, end_number).map_err(|err| RError::new(RErrorKind::InputOutputError, &err.to_string()));

        return order;
    }
    return Err(RError::new(RErrorKind::InputOutputError, "Can't read Order."));
}


//create_order
pub fn create_order(start_pos:u64, end_pos:u64, chunk_size:usize, file_hash_alg:&Hash, file_hash:&str, file_name:&str, _chunk_hash_type:&Option<Hash>) -> Result<Vec<u8>, Error>{
...
pub fn create_order_from_offer(chunk_size:usize, file_hash_alg:&Hash, chunk_hash_type:&Option<Hash>, output_dir:&str, offer:&Offer) -> Result<Vec<u8>, RError>{
    ...
    let mut order_byte_vec = create_order(1, max_count, chunk_size, &file_hash_alg, &offer.file_hash, &offer.name, &chunk_hash_type).map_err(|err| RError::new( RErrorKind::ConvertionError, &format!("Can't create offer-byte-vector. Error: {}", err)))?;

    order_byte_vec = append_header(order_byte_vec, HeaderByte::SendOrder);

    let _file_name = create_file_with_order(&output_dir,&offer.file_hash, &offer.name).map_err(|err| RError::new(RErrorKind::RegexError, &err.to_string()))?;
...
pub fn create_order_from_logfile(path:&str, buffer_size:usize, chunk_size:usize) -> Result<Vec<u8>, Error>{
    ...
    let mut file_name = PathBuf::from(path).file_name().unwrap().to_string_lossy().to_string();

    if file_name.ends_with(".rdroplog") {
        file_name.truncate(file_name.len() - 4);
    }

    let byte_vec = create_order(start_pos, end_pos, chunk_size, file_hash_alg, file_hash, &file_name, chunk_hash_type)?;
